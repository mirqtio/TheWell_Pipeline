    ...
    try {
      if (usesConcurrently) {
        ...
        try {
          ...
          logger.error(`[Migration ${version} (${name})] COMMIT Non-Concurrent Transaction`);
          logger.info(`[Migration ${version} (${name})] COMMIT transaction`); // Attempt to clear any aborted transaction state
        ...
        if (forwardScript) {
          ...
          for (const stmt of statements) {
            ...
            logger.info(`[Migration ${version} (${name})] Executing statement ${statements.indexOf(stmt) + 1}/${statements.length}: ${stmt.substring(0, 100)}...`);
            try {
              logger.error(`[Migration ${version} (${name})] Executing NC Stmt ${statements.indexOf(stmt) + 1}/${statements.length}: ${stmt.substring(0, 150).replace(/\n/g, ' ')}...`);
              try {
                await client.query(stmt);
                logger.info(`[Migration ${version} (${name})] NC Stmt ${statements.indexOf(stmt) + 1} executed successfully.`);
              } catch (stmtError) {
                logger.error(`[Migration ${version} (${name})] ERROR executing NC Stmt ${statements.indexOf(stmt) + 1}: ${stmtError.message}`);
                logger.error(`[Migration ${version} (${name})] Failing NC Stmt: ${stmt}`);
                throw stmtError;
              }
              logger.info(`[Migration ${version} (${name})] Statement ${statements.indexOf(stmt) + 1} executed successfully.`);
            } catch (stmtError) {
              logger.error(`[Migration ${version} (${name})] ERROR executing statement ${statements.indexOf(stmt) + 1}: ${stmtError.message}`);
              logger.error(`[Migration ${version} (${name})] Failing statement: ${stmt}`);
        ...
        logger.debug(`[Migration ${version} (${name})] BEGIN Non-Concurrent Transaction`);
        logger.info(`[Migration ${version} (${name})] BEGIN transaction`);
        await client.query(
        ...
        logger.debug(`[Migration ${version} (${name})] BEGIN Non-Concurrent Transaction`);
        if (forwardScript) {
          logger.debug(`[Migration ${version} (${name})] Executing Non-Concurrent Script (first 300 chars): ${forwardScript.substring(0, 300).replace(/\n/g, ' ')}...`);
          try {
            await client.query(forwardScript);
            logger.info(`[Migration ${version} (${name})] Non-Concurrent Script executed successfully.`);
          } catch (scriptError) {
            logger.error(`[Migration ${version} (${name})] ERROR executing Non-Concurrent Script: ${scriptError.message}`);
            const scriptToLog = forwardScript.length > 1000 ? forwardScript.substring(0, 1000) + '... (truncated)' : forwardScript;
            logger.error(`[Migration ${version} (${name})] Failing Non-Concurrent Script: ${scriptToLog}`);
            throw scriptError;
          }
        }
        await client.query(
        ...
        await client.query('COMMIT');
        logger.debug(`[Migration ${version} (${name})] COMMIT Non-Concurrent Transaction`);
        ...
        try {
          if (forwardScript && forwardScript.includes('CREATE TABLE e2e_test_documents')) {
            ...
            logger.debug(`[Migration ${version} (${name})] POST-COMMIT CHECK for e2e_test_documents: ${JSON.stringify(checkResult.rows[0])}`);
          }
          if (forwardScript && forwardScript.includes('CREATE TABLE integrity_test')) {
            ...
            logger.debug(`[Migration ${version} (${name})] POST-COMMIT CHECK for integrity_test: ${JSON.stringify(checkResult.rows[0])}`);
          }
        } catch (diagError) {
          logger.error(`[Migration ${version} (${name})] POST-COMMIT DIAGNOSTIC QUERY FAILED: ${diagError.message}`);
        }
        // **** END NEW DIAGNOSTIC ****

        try {
          await client.query('DISCARD ALL;');
          logger.debug(`[Migration ${version} (${name})] Session state discarded after non-concurrent commit.`);
        } catch (discardError) {
          logger.error(`[Migration ${version} (${name})] FAILED to DISCARD ALL: ${discardError.message}`);
          // Decide if this should be a fatal error for the migration
        }
      }
      logger.info(`[Migration ${version} (${name})] Successfully applied and recorded.`);
    } catch (error) {
      ...
      if (client && client.activeQuery === null) { // Check if client is not busy and can issue rollback
        try {
          ...
          logger.error(`Rollback attempt failed for ${version} (${name}): ${rbError.message}`);
        }
      }
      logger.error(`[Migration ${version} (${name})] Overall failure: ${error.message}`);
